...
analyzeBasisFunctions <- function(sp.obj = NULL, 
  ...
  # estimate & QR-decompose basis function matrix with user-specified kernel 
  M <- nrow(spatial_mtx)
  phi_raw <- matrix(0, 
                    nrow = M, 
                    ncol = n.basis.fns)
  ...
  # compute full kernel matrix & compare with low-rank basis function approximation
  ...
  for (i in seq(M)) {
    ...
    if (kernel == "exp_quad") {
      K[, i] <- expQuadKernel(dist_vec^2, length.scale = lscale)
    } else if (kernel == "matern") {
