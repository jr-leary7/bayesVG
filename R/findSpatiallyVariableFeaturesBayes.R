...
#' \item The length-scale \eqn{\ell} can be estimated in two ways. The default method is to perform \emph{k}-means clustering on the spatial coordinates then take the median of the Euclidean distances between the \emph{k} cluster centroids. The second method is to use aggregated variograms fit using \code{\link[gstat]{variogram}}; this method tends to be slightly slower, and generally estimates smaller values of \eqn{\ell}.
