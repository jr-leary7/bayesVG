...
#' \item The global length-scale \eqn{\ell} can be estimated in two ways. The default method is to perform \emph{k}-means clustering on the spatial coordinates then take the median of the Euclidean distances between the \emph{k} cluster centroids. The second method is to use aggregated variograms fit using \code{\link[gstat]{variogram}}; this method tends to be slightly slower, and generally estimates smaller values of \eqn{\ell}.
#' \item The argument \code{mle.init} is used to specify whether or not optimization should be performed via the L-BFGS algorithm in order to find the regularized maximum likelihood estimate (MLE) of each parameter, which is then used as initialization for the VI algorithm. This is sometimes unnecessary, but can help with convergence issues in large or complex datasets.
#' \item While we have implemented GPU acceleration via OpenCL through the argument \code{opencl.params}, OpenCL acceleration is not supported on every machine. For example, Apple M-series chips do not support double-precision floating point values, which are necessary for Stan to compile with OpenCL support. For more information, see \href{https://discourse.mc-stan.org/t/gpus-on-mac-osx-apple-m1/23375/5}{this Stan forums thread}. In order to correctly identify the OpenCL platform and device IDs, use the \code{clinfo} command line utility (available on GitHub).
...
findSpatiallyVariableFeaturesBayes <- function(sp.obj = NULL,
  ...
  if (!likelihood %in% c("gaussian", "nb")) { cli::cli_abort("Please provide a valid likelihood.") }
  ...
  if (inherits(sp.obj, "Seurat")) {
    expr_mtx <- Seurat::GetAssayData(sp.obj,
                                     ...
                                     layer = ifelse(likelihood == "gaussian", "data", "counts"))
  } else {
  ...
  if (subsample) {
    if (inherits(sp.obj, "Seurat")) {
      ...
      sp.obj@metadata$subsample_IDs <- sampled_spots
    }
  }
  if (save.model) {
    if (inherits(sp.obj, "Seurat")) {
  ...
  # finish & save runtime tracking
  time_diff <- Sys.time() - time_start
  if (inherits(sp.obj, "Seurat")) {
    sp.obj@assays[[Seurat::DefaultAssay(sp.obj)]]@misc$runtime <- time_diff
  } else {
    sp.obj@metadata$runtime <- time_diff
  }
